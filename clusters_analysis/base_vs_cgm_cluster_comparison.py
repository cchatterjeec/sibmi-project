...
(clustering_base.py) while refining it?

Crosstab + Adjusted Rand Index between the two KMeans labelings, restricted
to the participants present in both (those with usable CGM data).
...
# Where did the base beta-cell-exhaustion cluster (cluster 2, n=102) end up with CGM added?
beta_cell_base = merged[merged["kmeans_cluster_base"] == 2]
print(f"\nBase cluster 2 (beta-cell-exhaustion, n={len(beta_cell_base)}) -> with-CGM cluster distribution:")
print(beta_cell_base["kmeans_cluster_cgm"].value_counts().sort_index())
