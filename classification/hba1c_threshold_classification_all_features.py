...
(same cohort as run_classification_all_features.py in this directory) --
but here the label comes from thresholding a regressor's continuous HbA1c
prediction at 5.7 (healthy < 5.7 <= pre-diabetic, per the ADA prediabetes
cutoff), instead of training a classifier on study_group directly.

Same ablation stages, ElasticNet/XGBoost regressors, and FULL CGM feature
catalog (66 features, minus gmi_pct) as
regression_all_features/run_hba1c_regression_ablation.py -- refit here on
the restricted cohort (not reused from that script's full-population fit).
...
This is a distinct analysis from run_classification_all_features.py in
this directory, which trains a classifier directly on the study_group
...
Outputs (in this directory):
  hba1c_threshold_classification_results_all_features.csv
  hba1c_threshold_classification_auroc_forest_all_features.png
"""
import os
import sys
...
def main():
    ...
    results_df.to_csv(os.path.join(OUT_DIR, "hba1c_threshold_classification_results_all_features.csv"), index=False)
    make_auroc_forest_plot(
        results_df, os.path.join(OUT_DIR, "hba1c_threshold_classification_auroc_forest_all_features.png"),
