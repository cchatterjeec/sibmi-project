...
feature set (21 features), for the HbA1c-threshold-derived classification
(healthy vs pre-diabetic, label = HbA1c >= 5.7) --
hba1c_threshold_classification_no_correlated_features.py.

"Best performing" = higher test-set AUROC between ElasticNet and XGBoost
regressors at this stage (ElasticNet: 0.769 vs XGBoost: 0.743, per
hba1c_threshold_classification_results_no_correlated_features.csv) -- both
are fit here and the better one is plotted. AUROC/ROC use the continuous
predicted HbA1c as the score (no thresholding needed for ranking).

Output: hba1c_threshold_roc_curve_model6a_curated.png
"""
import os
import sys
...
for model_name, fitter in fitters.items():
    ...
    fpr, tpr, _ = roc_curve(y_test_class, pred_test)
...
ax.set_title(
    f"ROC curve: {stage_name}\nHbA1c-threshold (5.7), CGM: no correlated features",
...
fig.tight_layout()
fig.savefig(os.path.join(OUT_DIR, "hba1c_threshold_roc_curve_model6a_curated.png"), dpi=200, facecolor=fig.get_facecolor())
print("saved hba1c_threshold_roc_curve_model6a_curated.png")
