"""
Glycemic-variable ablation study runner for insulin prediction
(full covariate baseline, minus C-peptide).

Identical to ablation_runner_full.py, except C-peptide
("import_c_peptide, C peptide [Mass/volume] in Seru") is also excluded from
the baseline (and therefore from every scenario, since it's never added
back). C-peptide is co-secreted with insulin and strongly correlated with
it, so it's pulled out here to see whether the glycemic variables still add
predictive value once that confound is removed.
...
mean-of-nightly-means, MAGE, MODD) and C-peptide. Each subsequent scenario
adds exactly one glycemic variable back on top of that baseline, to isolate
how much each one individually adds when predicting insulin.

Usage (import as a function):

    from ablation_runner_no_cpeptide import run_ablation, build_default_feature_sets
    import pandas as pd

    df = pd.read_csv("final_df.csv")
    feature_sets = build_default_feature_sets(df)
    run_ablation(feature_sets, df=df, output_dir="results_no_cpeptide")

Usage (command line, e.g. on O2 via sbatch run_ablation_no_cpeptide.sh):

    python3 ablation_runner_no_cpeptide.py --data final_df.csv --output-dir results_no_cpeptide
"""
...
EXCLUDED_NON_GLYCEMIC = [
    ...
    "import_c_peptide, C peptide [Mass/volume] in Seru",  # excluded from baseline: co-secreted with insulin
]
...
def run_ablation(
    ...
    output_dir="results_no_cpeptide",
    target_col=TARGET_COL,
    cv=5,
    random_state=42,
    min_group_size=20,
...
def main():
    ...
    parser.add_argument("--output-dir", default="results_no_cpeptide", help="Directory to write results to")
