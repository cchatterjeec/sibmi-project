"""
Stage 0 (with CGM): KMeans + hierarchical (Ward) clustering on the 9-variable
diabetes-relevant panel (clustering_base.py) + 20 curated CGM features
(regression_no_correlated_features/select_cgm_features_vif.py). PCA fit
with 3 components. Requires the standard qc_pct_active >= 70% CGM quality
filter.
...
def main():
    ...
    fig.suptitle(f"Stage 0 (with CGM): 3D PCA projection (9-var panel + {len(CGM_FEATURES_VIF_PRUNED)} curated CGM features)", fontsize=13)
    fig.tight_layout()
