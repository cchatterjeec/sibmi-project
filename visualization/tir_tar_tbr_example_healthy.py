...
from the healthy study_group instead of insulin_dependent -- meant to sit
alongside that plot and tir_tar_tbr_example_prediabetic.py as a 3-way
"what does a normal/borderline/dysregulated CGM trace actually look like"
comparison.

Participant chosen to be representative of the healthy group rather than
a strict median: healthy participants are overwhelmingly >95% TIR with
near-zero TAR/TBR (median TAR here is 0.8%, IQR 0.2-2.2%), so a literal
median trace is close to a flat line at ~100 mg/dL and shows nothing.
Participant 1060 (TIR=97.0%, TAR=3.0%, TBR=0.1%) sits just above the 75th
percentile of TAR for this group -- still squarely typical of "healthy",
but with enough real excursion above 180 mg/dL (e.g. after meals) to be
visible on the plot, full ~10-day/2843-reading wear.

Output: tir_tar_tbr_example_healthy.png
"""
import os
...
def main():
    ...
    fig.tight_layout()
    out_path = os.path.join(HERE, "tir_tar_tbr_example_healthy.png")
