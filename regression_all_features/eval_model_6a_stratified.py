...
ElasticNet and XGBoost) by subgroup: sex, race, BMI category, and diabetes
status (study_group).

gmi_pct is excluded for the same reason as run_hba1c_regression_ablation.py
in this directory: it's an algebraic estimated-A1c formula, near-tautological
as an HbA1c predictor. mean_glucose is kept (genuine physiology, not leakage).
...
def main():
    ...
    make_strat_forest_plot(
        all_results, os.path.join(OUT_DIR, "hba1c_strat_forest.png"),
        title_prefix="Model 6a HbA1c $R^2$ by",
    )
