...
using the FULL CGM ML feature catalog (cgm_ml_features.py -- 67 features,
gmi_pct INCLUDED) as the Model 5a/6a CGM block. c-peptide is excluded
throughout (co-secreted with insulin -> leakage).

gmi_pct is kept here (unlike the HbA1c scripts in this same directory):
it's an algebraic estimated-A1c formula, so it's near-tautological as a
predictor of lab HbA1c, but it's an ordinary (if collinear-with-mean_glucose)
CGM summary statistic with no special relationship to insulin.
...
def main():
    ...
    stages = get_ablation_stages(
        df, exclude_cpeptide=True, include_no_labs_variants=True, cgm_features=CGM_ML_FEATURES,
    )
