"""
Minimal-covariate ablation study runner for insulin prediction.

Same 3-rung glucose ablation ladder as ablation_runner.py, but the
non-glucose "base" feature set is restricted to just BMI, age, sex, and race
(instead of every other available lab/vital/activity variable). Useful for
isolating how much glucose-derived features add on top of basic demographics
alone, rather than on top of the full clinical panel.
...
Usage (import as a function):
    ...
    from ablation_runner_minimal import run_ablation, build_default_feature_sets
    import pandas as pd

    df = pd.read_csv("final_df.csv")
    feature_sets = build_default_feature_sets(df)
    run_ablation(feature_sets, df=df, output_dir="ablation_results_minimal")

Usage (command line, e.g. on O2 via sbatch run_ablation_minimal.sh):

    python3 ablation_runner_minimal.py --data final_df.csv --output-dir ablation_results_minimal
"""
...
# Only these four non-glucose covariates are used as the "base" feature set.
BASE_FEATURES = ["age", "bmi_vsorres, BMI", "sex", "race"]
...
def build_default_feature_sets(df):
    """
    Builds the 3-rung glucose ablation ladder for insulin prediction, with
    the base (non-glucose) feature set restricted to age, BMI, sex, race:

      1. no_glucose      -- BASE_FEATURES only, no glucose vars at all
      ...
      3. labs_plus_cgm    -- add back CGM metrics on top of (2): the "add CGM
                             ...
                             tar/tbr are excluded from every scenario entirely.
    """
...
def run_ablation(
    ...
    output_dir="ablation_results_minimal",
    target_col=TARGET_COL,
    cv=5,
    random_state=42,
    min_group_size=20,
...
def main():
    parser = argparse.ArgumentParser(
        description="Run insulin-prediction glucose-feature ablation studies "
                     "with a minimal (age/BMI/sex/race only) base feature set."
    )
    parser.add_argument("--data", default="final_df.csv", help="Path to input CSV")
    parser.add_argument("--output-dir", default="ablation_results_minimal", help="Directory to write results to")
