...
cumulative feature-ablation stages, using the VIF-pruned CGM feature set
(select_cgm_features_vif.py -- 23 of the 67 catalog features survive VIF
<= 10, computed on the CGM block alone; nothing else in the model is
pruned). gmi_pct and mean_glucose are both already gone at this VIF
threshold (they're ~perfectly collinear with each other and with several
other central-tendency columns), so no separate gmi_pct exclusion is needed
here the way it is in regression_all_features/.
